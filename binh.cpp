...
//setting value of days of each month
...
void inputInfo(HealthProfile& person) {
    ...
    while (lower(gender) != "male" && lower(gender) != "female" && lower(gender) != "other") {
        cout << "Error: Gender must be 'Male', 'Female', or 'Other'. " << endl << "Enter again: ";
        getline(cin, gender);
    }
}
...
void inputDate(HealthProfile& person, int currentYear) {
    ...
    while (!person.setDate(day, month, year)) {
        int maxDays = person.getMaxDays();
        cout << "Error: Day must be between 1 and " << maxDays << " for month " << month << " of year " << year << ".\n";
        cout << "Please re-enter day (1-" << maxDays << "): ";
        while (!(cin >> day) || day < 1 || day > maxDays) {
            cout << "Error: Day must be between 1 and " << maxDays << ". Try again: ";
            cin.clear();
            cin.ignore(10000, '\n');
        }
    }
...
int main() {
    ...
    HealthProfile person("Temp", "Temp", "Other", 1, 1, 1900, 1.0, 50.0);
